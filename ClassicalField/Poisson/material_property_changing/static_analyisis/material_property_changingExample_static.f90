...
!=== Flux of current at the edge of the electrodes:

!===Returning electrode
  !DO i=1,8
 !   CALL CMISSBoundaryConditions_SetNode(BoundaryConditions,DependentField,CMISS_FIELD_U_VARIABLE_TYPE,1,1, &
  !    & returning_electrode(i),1,CMISS_BOUNDARY_CONDITION_NEUMANN_INTEGRATED,returning,Err)
 ! ENDDO

  DO i=1,8
    CALL CMISSBoundaryConditions_SetNode(BoundaryConditions,DependentField,CMISS_FIELD_DELUDELN_VARIABLE_TYPE,1,1, &  
      & returning_electrode(i),1,CMISS_BOUNDARY_CONDITION_NEUMANN_POINT,returning,Err)
  ENDDO
!==== Injecting electrode
  !DO i=1,8
  !  CALL CMISSBoundaryConditions_SetNode(BoundaryConditions,DependentField,CMISS_FIELD_U_VARIABLE_TYPE,1,1, &
  !    & injecting_electrode(i),1,CMISS_BOUNDARY_CONDITION_NEUMANN_INTEGRATED,injecting,Err)
 ! ENDDO

  DO i=1,8
  CALL CMISSBoundaryConditions_SetNode(BoundaryConditions,DependentField,CMISS_FIELD_DELUDELN_VARIABLE_TYPE,1,1, &
    & injecting_electrode(i),1,CMISS_BOUNDARY_CONDITION_NEUMANN_POINT,injecting,Err)
  ENDDO
!====
